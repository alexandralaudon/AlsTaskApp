...
describe TasksController do
  ...
  describe 'PERMISSIONS' do
    ...
    describe '#require_login' do
      it 'redirects user who is not logged in to the sign-in-path' do
        session[:user_id] = nil
        membership = create_membership(@project, @user, role: 'Member')
        get :index, project_id: @project.id
        expect(response).to redirect_to sign_in_path
      end
      it 'lets a logged-in user see projects#index' do
        membership = create_membership(@project, @user, role: 'Member')
        get :index, project_id: @project.id
        expect(response).to render_template(:index)
      end
    ...
    describe '#ensure_owner_or_member' do
      it 'redirects user to projects_path if not a owner or member' do
        user = create_user(email: 'akjshdf@lsjdk.net')
        task = create_task(@project)
        membership = create_membership(@project, user, role: 'Member')
        get :show, project_id: @project.id, id: task.id
        expect(response).to redirect_to projects_path
      end
      it 'allows a user to see tasks if he/she is an owner or member of the overall project' do
        task = create_task(@project)
        membership = create_membership(@project, @user, role: 'Member')
        get :show, project_id: @project.id, id: task.id
        expect(response).to render_template(:show)
