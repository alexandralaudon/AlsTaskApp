...
describe UsersController do
  ...
  describe 'PERMISSIONS' do
    describe '#require_login' do
      it 'expects a user will be rerouted if user is not signed in' do
        get :index
        expect(response).to redirect_to sign_in_path
      end
      it 'expects a logged-in user to see the users index' do
    ...
    describe '#ensure_current_user_cannot_change' do
      it 'expects current user to not change other users information' do
        user1 = create_user(admin:false)
        user2 = create_user(email: 'kasjdhf@ksjhdf.com')
